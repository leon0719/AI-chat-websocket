...
IGNORED_FUNCTIONS = {
    ...
    # Django storage methods
    "get_available_name",
    "get_valid_name",
    # Django Ninja parser methods
    "parse_body",
    ...
    # Django Channels WebSocket Consumer methods
    "connect",
    "disconnect",
    "receive",
    "websocket_connect",
    ...
    "websocket_receive",
    # Django Ninja authentication methods (called by framework)
    "authenticate",
    # Common serialization methods (called externally)
    "to_dict",
    "to_json",
...
IGNORED_CLASSES = {
    # Django model mixins (inherited, not instantiated)
    "CreatedAtMixin",
    "UpdatedAtMixin",
    "LanguageMixin",
    "FileMixin",
    ...
    "Consumer",
    # Dataclasses used for type hints
    "StreamChunk",
    "StreamError",
}
...
EXTERNAL_USE_DECORATORS = {
    ...
    "task",
    "shared_task",
    # Celery/Django signal decorators (e.g., @before_task_publish.connect)
    "connect",
    # Context managers
    "contextmanager",
    "asynccontextmanager",
...
def extract_references(file_path: Path) -> set[str]:
    ...
    try:
        ...
        for node in ast.walk(tree):
            ...
            # e.g., list_display = ("user", "paid_type", "e_date")
            # e.g., MIDDLEWARE = ["chat_console_3.middlewares.add_slash"]
...
def find_unused_symbols(
    ...
    # Collect all references from ALL files (including models.py)
